"""PathoMind dashboard - multi-page Streamlit application.
...
def _page_medical_demo_evaluation() -> None:
    from src.observability.dashboard.pages.medical_demo_evaluation import render

    render()


pages = [
    st.Page(_page_overview, title="Medical Demo", icon="🩺", default=True),
    st.Page(_page_data_browser, title="Knowledge Base", icon="🔍"),
    st.Page(_page_ingestion_manager, title="Ingestion Center", icon="📥"),
    st.Page(_page_ingestion_traces, title="Ingestion Traces", icon="🧩"),
    st.Page(_page_query_traces, title="Query Diagnostics", icon="🔎"),
    st.Page(_page_medical_demo_evaluation, title="Medical Demo Evaluation", icon="🏥"),
    st.Page(_page_evaluation_panel, title="General Evaluation", icon="📏"),
]


def main() -> None:
    st.set_page_config(
        page_title="PathoMind Medical Demo",
        page_icon="🩺",
        layout="wide",
    )
